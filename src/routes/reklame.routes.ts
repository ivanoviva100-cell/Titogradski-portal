...
router.put('/:id', autentifikujKorisnika, zahtijevajAdmina, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { naziv, pozicija, slikaUrl, linkUrl, aktivna } = req.body;
...
router.delete('/:id', autentifikujKorisnika, zahtijevajAdmina, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
